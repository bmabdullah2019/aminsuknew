...
@section('title','Record Stock Loss')
@section('css')
...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">Record Stock Loss</h4>
            </div>
        </div>
    </div>       
    <!-- end page title --> 
   <div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{route('admin.loss.store')}}" id="lossForm">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-3">
                            <div class="form-group mb-3">
                                ...
                                <input type="date" name="loss_date" class="form-control" value="{{old('loss_date', date('Y-m-d'))}}" required>
                                @error('loss_date')
                                    <span class="text-danger">{{$message}}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-3">
                            <div class="form-group mb-3">
                                ...
                                <select name="loss_type" class="form-control" required>
                                    ...
                                    <option value="damage" {{old('loss_type')=='damage'?'selected':''}}>Damage</option>
                                    <option value="expiry" {{old('loss_type')=='expiry'?'selected':''}}>Expiry</option>
                                    <option value="theft" {{old('loss_type')=='theft'?'selected':''}}>Theft</option>
                                    <option value="missing" {{old('loss_type')=='missing'?'selected':''}}>Missing</option>
                                    <option value="quality_issue" {{old('loss_type')=='quality_issue'?'selected':''}}>Quality Issue</option>
                                    <option value="other" {{old('loss_type')=='other'?'selected':''}}>Other</option>
                                </select>
                                @error('loss_type')
                                    <span class="text-danger">{{$message}}</span>
                                @enderror
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <a href="{{route('admin.loss.index')}}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    $('#add-item').click(function() {
        ...
        const line = { product_id: productId, product_label: productLabel, quantity: qty, notes: notes || '' };

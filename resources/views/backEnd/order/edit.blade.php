...
@enderror
                                ...
                                <div class="col-sm-12">
                                    <div class="form-group mb-2">
                                        <input type="text" id="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="Customer Number" name="phone" value="{{$shippinginfo->phone}}"  required>
                                        @error('phone')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                        @enderror
                                        <div id="phone-block-alert" class="alert alert-danger mt-2 mb-0 py-2 d-none"></div>
                                    </div>
                                </div>
                                <!-- col-end -->
                                <div class="col-sm-12">
                                    <div class="form-group mb-3">
                                        <input type="text" placeholder="Address" id="address" class="form-control @error('address') is-invalid @enderror" name="address" value="{{$shippinginfo->address}}"  required>
                                        @error('address')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                        ...
                        <div>
                            <input type="submit" id="order-submit-btn" class="btn btn-success" value="Update Order" />
                        </div>
                    </form>
                </div>
                <!-- end card-body-->
...
<script>
   ...
   $(".product_discount").change(function(){
    ...
    // Customer lookup by phone (auto fill shipping info)
    var customerLookupTimer = null;
    var lastLookupPhone = null;
    var $submitBtn = $("#order-submit-btn");

    function setPhoneBlockState(isBlocked, reason) {
        var $alert = $("#phone-block-alert");
        if (isBlocked) {
            $alert
                .removeClass("d-none")
                .text(reason || "This phone number is blocked for new orders.");
    ...
    function runCustomerLookup(phone) {
        lastLookupPhone = phone;
        $.ajax({
            type: "GET",
            url: "{{ route('admin.order.customer_lookup') }}",
            data: { phone: phone },
            dataType: "json",
            success: function (data) {
                if (!data) {
                    setPhoneBlockState(false);
                    return;
                }

                setPhoneBlockState(!!data.is_phone_blocked, data.phone_block_reason);

                if (!data.found) {
                    return;
                }

                var currentPhone = String($("#phone").val() || "").trim();
                if (currentPhone !== String(lastLookupPhone)) {
                    return;
                }
                ...
                if (data.area_id) {
                    $("#area").val(String(data.area_id)).trigger("change");
                } else if (data.area_name) {
                    var $area = $("#area");
                    var match = $area
                        .find("option")
                        .filter(function () {
                            return $(this).text().trim() === String(data.area_name).trim();
                        })
                        .first();
                    if (match.length) {
                        $area.val(match.val()).trigger("change");
                    }
                }
            },
        });
    ...
    $("#phone").on("input", function () {
        ...
        customerLookupTimer = setTimeout(function () {
            runCustomerLookup(phone);
        }, 400);
    });

    var initialPhone = String($("#phone").val() || "").trim();
    if (initialPhone.length >= 6) {
        runCustomerLookup(initialPhone);
    }

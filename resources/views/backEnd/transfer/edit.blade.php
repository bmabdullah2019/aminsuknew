...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">Edit Warehouse Transfer #{{$transfer->transfer_number}}</h4>
            </div>
        </div>
    </div>       
    <!-- end page title --> 
   <div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{route('admin.transfer.update', $transfer->id)}}" id="transferForm">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <input type="date" name="estimated_arrival" class="form-control" value="{{$transfer->estimated_arrival ? $transfer->estimated_arrival->format('Y-m-d') : ''}}">
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" name="reason" class="form-control" value="{{$transfer->reason}}" placeholder="Reason for transfer" required>
                                @error('reason')
                                    <span class="text-danger">{{$message}}</span>
                                @enderror
                            </div>
                    ...
                    <h5>Items</h5>
                    @php
                        $initialItems = $transfer->items->map(function ($i) {
                            return [
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <a href="{{route('admin.transfer.show', $transfer->id)}}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    const lines = (Array.isArray(initialItems) ? initialItems : []).map(function (item) {
        return { product_id: String(item.product_id || ''), product_label: '', quantity: item.quantity || '' };
    });
    ...
    function renderGrid() {
        ...
        lines.forEach(function (line, index) {
            const label = line.product_label || $('#builderProduct option[value="' + String(line.product_id) + '"]').text().trim() || '-';
            line.product_label = label === '-' ? '' : label;
            $grid.append(`
                <tr data-index="${index}">

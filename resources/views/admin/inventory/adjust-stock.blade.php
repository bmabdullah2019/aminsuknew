...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-md-12">
            <h1 class="mb-0">Adjust Stock</h1>
            <small class="text-muted">Fix inventory counting errors, damage, expiry, etc</small>
            <hr>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.inventory.store-adjust-stock') }}" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label><strong>Adjustment Type</strong></label>
                                    <select name="adjustment_type" class="form-control @error('adjustment_type') is-invalid @enderror" required>
                                        <option value="">-- Choose Type --</option>
                                        <option value="damage" {{ old('adjustment_type') == 'damage' ? 'selected' : '' }}>Damaged Goods</option>
                                        <option value="shortage" {{ old('adjustment_type') == 'shortage' ? 'selected' : '' }}>Shortage (Missing)</option>
                                        <option value="expiry" {{ old('adjustment_type') == 'expiry' ? 'selected' : '' }}>Expired</option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="reason" class="form-control" placeholder="Brief reason for adjustment" value="{{ old('reason') }}">
                        </div>

                        <hr class="my-4">
                        <h5>Items to Adjust</h5>
                        <p class="small text-muted">Positive numbers add stock, negative numbers remove stock.</p>
                            ...
                            <div class="item-row mb-3 p-3 border rounded">
                                <div class="row">
                                    <div class="col-md-3">
                                        ...
                                        <select name="items[0][product_id]" class="form-control product-select @error('items.0.product_id') is-invalid @enderror" required>
                                            ...
                                            @foreach ($products as $product)
                                                <option value="{{ $product->id }}" {{ (string) old('items.0.product_id') === (string) $product->id ? 'selected' : '' }}>
                                                    {{ $product->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" name="items[0][quantity]" class="form-control @error('items.0.quantity') is-invalid @enderror" placeholder="-5 or +10" step="0.01" required value="{{ old('items.0.quantity') }}">
                        ...
                        <div class="form-group">
                            <label>Notes</label>
                            <textarea name="notes" class="form-control" rows="3" placeholder="Detailed notes about this adjustment...">{{ old('notes') }}</textarea>
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <a href="{{ route('admin.inventory.index') }}" class="btn btn-secondary btn-lg">
        ...
        <div class="col-md-4">
            <div class="card bg-light">
                <div class="card-body">
                    <h5 class="card-title">Adjustment Examples</h5>
                    <div class="small">
                        <p><strong>Damage Found:</strong> Remove 5 units by entering <code>-5</code></p>
                        <p><strong>Found Extra:</strong> Add 3 units by entering <code>+3</code></p>
                        <p><strong>Expiry:</strong> Remove 10 units by entering <code>-10</code></p>
                        <hr>
                        <p class="mb-0"><strong>Tip:</strong> Always provide detailed notes for audit trail.</p>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    let itemCount = 1;
    const productVariantsEndpoint = 'api/product-variants';
    const variantCache = {};
    ...
    function optionsForGroup(variants, groups, selectedValues, group) {
        ...
        return (group.values || []).filter((value) => allowedValueIds.has(normalize(value.value_id)));
    }
    ...
    async function fetchProductVariants(productId) {
        ...
        const url = new URL(productVariantsEndpoint, window.location.href);
        ...
        const newItem = `
            <div class="item-row mb-3 p-3 border rounded">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <label>Quantity (+ or -)</label>
                        <input type="number" name="items[${itemCount}][quantity]" class="form-control" placeholder="-5 or +10" step="0.01" required>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('.item-row').forEach((row) => {
            ...
            const selectedVariantId = variantIdInput ? normalize(variantIdInput.dataset.selected || variantIdInput.value) : '';

            if (!productSelect || normalize(productSelect.value) === '') {
                setSimpleProductState(row, 'Select product first');
                return;
            }

            handleProductChange(productSelect, selectedVariantId);
        });
    });
</script>
@endsection

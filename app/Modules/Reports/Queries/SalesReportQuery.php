...
final class SalesReportQuery
{
    ...
    public function detailedLines(string $startDate, string $endDate, array $filters = []): Builder
    {
        $query = DB::table('order_details')
            ->join('orders', 'orders.id', '=', 'order_details.order_id')
            ->leftJoin('shippings', 'shippings.order_id', '=', 'orders.id')
            ->leftJoin('warehouses', 'warehouses.id', '=', 'order_details.warehouse_id')
            ->whereBetween('orders.created_at', [
                Carbon::parse($startDate)->startOfDay(),
                Carbon::parse($endDate)->endOfDay(),
            ])
            ...
            ->select(
                ...
                'orders.invoice_id',
                'orders.created_at',
                'shippings.name as customer_name',
                'shippings.phone as customer_phone',
                'order_details.product_name',
                'order_details.qty',
                'order_details.returned_quantity',
                'order_details.sale_price',
                'order_details.purchase_price',
                'order_details.sale_price_minor',
                'order_details.purchase_price_minor',
    ...
    {
        $saleMinorExpr = 'COALESCE(NULLIF(order_details.sale_price_minor, 0), order_details.sale_price * 100, 0)';
        $purchaseMinorExpr = 'COALESCE(NULLIF(order_details.purchase_price_minor, 0), order_details.purchase_price * 100, 0)';

        $mainQuery = $this->lines($startDate, $endDate, $filters);

        $summary = (clone $mainQuery)
            ->selectRaw("SUM(order_details.qty * {$saleMinorExpr}) as total_sales_minor")
            ->selectRaw("SUM(order_details.qty * {$purchaseMinorExpr}) as total_cogs_minor")
            ->selectRaw('SUM(order_details.qty) as total_items')
        ...
        if (Schema::hasTable('return_items')) {
            ...
            $returnsMinor = (float) $returnsQuery
                ->selectRaw('SUM(return_items.return_quantity * return_items.unit_price * 100) as total_return_minor')
                ->value('total_return_minor');
        }

        $summary->total_returns_minor = (float) $returnsMinor;
